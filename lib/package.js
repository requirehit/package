...
var Package = module.exports = Class.extend({
    ...
    initialize: function ( options ) {
        ...
        debug( '%s: initialize package', options.path );
    ...
    load: function ( force ) {
        ...
        debug( '%s: loading package', options.path );

        return this._loaded = Promise.cast().bind(this)
        .then(function () {
            var pj;

            debug( '%s: trying to load package.json first', options.path );

            try {
                pj = require( options.path + '/package.json' );
            } catch ( err ) {
    ...
    parseOptionsFromPackageJSON: function ( pj ) {
        ...
        debug( '%s: parsing package.json', options.path );
    ...
    setDependencies: function ( dependencies ) {
        ...
        debug( '%s: saving dependencies', options.path );
